...
• Use REDACTED_shell instead of IDE run_command — captures output internally
• Use REDACTED_filesystem (read_file/write_file/search_files) instead of IDE view_file/grep_search — returns structured data, supports batch reads via read_multiple
• Use code_analysis instead of manual grep for function/class/import extraction — instant structured AST output
• Use REDACTED_git instead of IDE git commands — structured JSON output, no parsing needed
• Use REDACTED_database for CSV/JSON/SQLite queries — SQL on files without scripts
• Use REDACTED_document_reader for batch document reading — read multiple files in one call
