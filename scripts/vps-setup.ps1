...
# ─── Step 3: Create vegamcp test directory ───
...
Run-Test "Create temp directory" { New-Item -ItemType Directory -Path "$env:TEMP\vegamcp-test" -Force }
Run-Test "Write file" { "Hello from VegaMCP" | Set-Content "$env:TEMP\vegamcp-test\test.txt" }
Run-Test "Read file" { Get-Content "$env:TEMP\vegamcp-test\test.txt" }
Run-Test "List directory" { Get-ChildItem "$env:TEMP\vegamcp-test" }
Run-Test "Delete temp dir" { Remove-Item "$env:TEMP\vegamcp-test" -Recurse -Force }
...
Write-Host "  then test: ssh trader@45.93.216.232" -ForegroundColor Yellow
Write-Host ""
