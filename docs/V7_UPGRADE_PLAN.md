...
**Output:** Each tool gets a corresponding `.claude/skills/REDACTED-{tool}.md` compatible file.
...
export function withResourceLinks(result: any, refs: ResourceReference[]) {
  return {
    ...result,
    _REDACTED_resources: refs.map(r => ({
      uri: r.uri,
      summary: r.summary,
      size: r.size,
...
// Agent Card for VegaMCP
{
  ...
  "url": "https://REDACTED.io/a2a",
  "provider": {
    "name": "VegaMCP",
    "url": "https://REDACTED.io"
  },
  "version": "7.0.0",
  "documentationUrl": "https://docs.REDACTED.io",
  "capabilities": {
    "streaming": true,
...
```bash
# Claude Code
claude mcp add REDACTED -- $(npm root -g)/REDACTED/build/index.js

# Codex CLI
codex mcp add REDACTED -- node $(npm root -g)/REDACTED/build/index.js

# Kimi Code
kimi mcp add --transport stdio REDACTED -- node /path/to/REDACTED/build/index.js

# Global npm (detects client)
npm install -g REDACTED
REDACTED install  # Auto-detects and configures for installed clients
```
...
export async function syncConfig(): Promise<void> {
  ...
  for (const [client, path] of Object.entries(configs)) {
    if (await fileExists(path)) {
      await mergeMcpConfig(path, 'REDACTED', REDACTEDConfig);
    }
  }
}
```
...
4. **Auto-Upgrade:** `REDACTED upgrade` handles configuration migration
